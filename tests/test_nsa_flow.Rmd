...
if ( ! exists("results") ) {
  ...
  for (i in 1:maxcfnfigs) {
    ...
    rep_results <- lapply(1:n_reps, function(rep) {
      v=FALSE
      mb <- microbenchmark(
        res <- ANTsR::nsa_flow( data$Y0, X0=X0_use, w=cfg$w, 
...
## Relative Fidelity 
```{r relfid,fig.width=10, fig.height=3.5}
...
ggplot(tradeoff_df_unsigned, aes(x = w, y = rel_fid, color = dataset, group = dataset)) +
  ...
  labs(title = "Relative Fidelity by Retraction and Dataset: (+)",
       y = "Relative Fidelity", x = "Tradeoff weight w") +
  theme_minimal(base_size = 13) +
  coord_cartesian(ylim = c(0.35, 1.1))   # <── limits y-axis to 0–2
...
```{r ordef,fig.width=10, fig.height=4}
...
```{r rel_orth,fig.width=10, fig.height=4}
if ( nrow(tradeoff_df_signed > 0)) 
