...
if ( ! exists("results") ) {
  ...
  for (i in 1:maxcfnfigs) {
    ...
    rep_results <- lapply(1:n_reps, function(rep) {
      v=FALSE
      mb <- microbenchmark(
        res <- nsa_flow( data$Y0, X0=X0_use, w=cfg$w, 
          retraction=cfg$retraction,  max_iter=cfg$max_iter, optimizer='fast',
          initial_learning_rate='armijo',
...
```{r timing_data_collection,echo=FALSE,eval=TRUE,cache=FALSE}
library(ggplot2)
library(dplyr)
library(tidyr)
...
```{r timing_data_collection_plot,echo=FALSE,eval=TRUE,cache=FALSE}
# --- Plot timing results ---
