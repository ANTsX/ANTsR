...
if ( ! exists("results") ) {
  ...
  for (i in 1:maxcfnfigs) {
    ...
    rep_results <- lapply(1:n_reps, function(rep) {
      v=FALSE
      mb <- microbenchmark(
        res <- nsa_flow_autograd( data$Y0, X0=X0_use, w=cfg$w, 
          retraction=cfg$retraction,  max_iter=cfg$max_iter, 
          optimizer='asgd', lr_strategy='armijo',
          initial_learning_rate=NULL,
          warmup_iters = 10,
          apply_nonneg=cfg$apply_nonneg, 
          seed=42+rep, verbose=v, plot=TRUE ),
        times=1
      )
