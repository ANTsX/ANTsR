    ...
    rep_results <- lapply(1:n_reps, function(rep) {
      mb <- microbenchmark(
        res <- ANTsR::nsa_flow( data$Y0, X0=X0_use, w=cfg$w, retraction=cfg$retraction, 
          optimizer='adam',
          initial_learning_rate=1e-3, max_iter=1000, tol=1e-4,
          apply_nonneg=cfg$apply_nonneg, seed=42+rep, verbose=TRUE, plot=TRUE),
        times=3
      )
...
ggplot(tradeoff_df, aes(x = w, y = rel_fid, color = dataset, group = dataset)) +
  ...
  coord_cartesian(ylim = c(0.45, 1.05))   # <── limits y-axis to 0–2
