...
test_that("dim of antsImage", {
  expect_true(sum(dim(img)==c(10,10))==2)
})

test_that("dim of as.array", {
  expect_true( sum(dim(as.array(img))==c(10,10))==2 )
})
...
test_that( "antsGetSpacing works", {
  expect_true( sum(antsGetSpacing(img) == c(2,3)) == 2 )
})

test_that( "antsGetOrigin works", {
  expect_true( sum(antsGetOrigin(img) == c(4,5)) == 2 )
})

test_that( "antsGetDirection works", {
  expect_true( sum(antsGetDirection(img) == -diag(2)) == 4 )
})
...
test_that( "antsImage + antsImage, preserves header", {
  expect_true( antsImagePhysicalSpaceConsistency(data.type=T,img, img) )
})
...
test_that( "antsImage * antsImage, preserves header", {
  expect_true( antsImagePhysicalSpaceConsistency(data.type=T,img, img*img) )
})
...
test_that( "antsImage / antsImage, preserves header", {
  expect_true( antsImagePhysicalSpaceConsistency(data.type=T,img1, img1/img3) )
})
...
test_that( "antsImage ^ antsImage, preserves header", {
  expect_true( antsImagePhysicalSpaceConsistency(data.type=T,img3, img3^img3) )
})
...
test_that( "antsImage log, preserves header", {
  expect_true( antsImagePhysicalSpaceConsistency(data.type=T,img3, log(img3)) )
})
...
test_that( "antsImage exp, preserves header", {
  expect_true( antsImagePhysicalSpaceConsistency(data.type=T,img, exp(img)) )
})
...
test_that( "antsImage exp, preserves header", {
  expect_true( antsImagePhysicalSpaceConsistency(data.type=T,img, abs(img)) )
})
...
test_that("dim of as.array on multichannel", {
  expect_true( sum(dim(as.array(mimg))==c(2,10,10))==3 )
})
...
test_that( "antsImage + scalar, preserves header on multichannel", {
  expect_true( antsImagePhysicalSpaceConsistency(data.type=T,mimg1, mimg1 + 2) )
})
...
test_that( "antsImage /+ antsImage, preserves header on multichannel", {
  expect_true( antsImagePhysicalSpaceConsistency(data.type=T,mimg1, mimg1+mimg3) )
})
...
test_that( "antsImage / scalar, preserves header on multichannel", {
  expect_true( antsImagePhysicalSpaceConsistency(data.type=T,mimg, mimg / 2) )
})
...
test_that( "antsImage ^ scalar, preserves header on multichannel", {
  expect_true( antsImagePhysicalSpaceConsistency(data.type=T,mimg3, mimg3^2) )
})
...
test_that( "antsImage ^ antsImage, preserves header on multichannel", {
  expect_true( antsImagePhysicalSpaceConsistency(data.type=T,mimg3, mimg3^mimg3) )
})
...
test_that( "antsImage log, preserves header on multichannel", {
  expect_true( antsImagePhysicalSpaceConsistency(data.type=T,mimg3, log(mimg3)) )
})
...
test_that( "antsImage exp, preserves header on multichannel", {
  expect_true( antsImagePhysicalSpaceConsistency(data.type=T,mimg3, exp(mimg3)) )
})
...
test_that( "antsImage exp, preserves header on multichannel", {
  expect_true( antsImagePhysicalSpaceConsistency(data.type=T,mimg, abs(mimg)) )
})
