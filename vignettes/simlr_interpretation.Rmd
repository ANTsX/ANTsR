...
npix <- c(50, 200, 93)  # Number of features in each view
nk <- 4             # Number of latent factors
...
outcome1 = gsmd$ground_truth[[2]]$View1
...
A more complex data generation scheme is available in `generate_structured_multiview_data`.
...
constr='nsaflowx0.5x10' # good default
...
p2 <- mat2 %*% (result$v[[2]])
p3 <- mat3 %*% (result$v[[3]])
# pheatmap::pheatmap( result$v[[1]], cluster_cols=F, cluster_rows=F )
```
...
library(ggpubr)
sim2nm=tempfile()
sim2nmr=paste0(sim2nm,"_simlr") 
write_simlr( result, sim2nm )
simres2=read_simlr( sim2nmr )
popdf = data.frame( age = outcome, cog=outcome1, mat1, mat2, mat3 )
temp2=apply_simlr_matrices( popdf, simres2$v )
simnames=temp2[[2]]
...
myperm = simlr.perm(  matlist,
  ...
  mixAlg='pca', # recommended
  initialUMatrix=initu,
  iterations=50,
  nperms=np, FUN=rvcoef, verbose=FALSE )
...
for ( i in 1:(length(matlist)-1)) {
    for ( j in ((i+1):length(matlist))) {
        toviz1=data.matrix(simres2$v[[i]])
        toviz2=data.matrix(simres2$v[[j]])
        temp=visualize_lowrank_relationships( matlist[[i]], matlist[[j]], 
            toviz1, toviz2,
            nm1=names(matlist)[i], nm2=names(matlist)[j] )$heatmap
...
csearch = list( "nsaflowx0.05x50", "nsaflowx0.5x50")
...
simparms = simlr.parameters( 
    ...
    objectiver_options = list('acc','normalized_correlation', "regression"),
    ...
    search_type="random",  # full or random
    num_samples=5
    )
``` 
...
performance_data <- simlrXs_paramsearch %>%
  ...
  select(
    ...
    Optimizer = optimus,
    RV_12 = rvcoef_View1_View2,
    RV_13 = rvcoef_View1_View3,
    RV_23 = rvcoef_View2_View3
  ) %>%
...
plot_ranking <- ggplot(
  ...
  scale_shape_manual(name = "Mixer", values = c("ica" = 17, "pca" = 16, "svd"=15)) +
