...
## Preprocessing
The preprocessing steps include processing methods used for processing fMRI for
a variety of purposes. This is followed by a section of Connectitivy processing
that includes steps that are specific to processing resting state data.
...
non image based physiological measures, if acquired. In the plot below, mean global
signal in the brain is plotted with a red line indicating the point at which the
system has reached the steady state. Data previous to this point are discarded.

```{r ssplot,message=FALSE,warnings=FALSE, fig.width=7, fig.height=3, echo=FALSE}
origmean = apply.antsImage(img, c(1,2,3), mean)
xintercept = 10*spacing[4]

ss.dat <- data.frame(Time=rep(1:allTimes)*spacing[4])
ss.dat$Values = fullmean
ssPlot <- ggplot(ss.dat, aes(x=Time, y=Values))
  ssPlot <- ssPlot + geom_line(size=0.5)
  ssPlot <- ssPlot + geom_vline( xintercept=xintercept,colour="red" )
  ssPlot <- ssPlot + theme(text=element_text(size=10), legend.position="top")
  ssPlot <- ssPlot + ggtitle("exclude non steady state points")
print(ssPlot)
...
* Regress out nuissance parameters
...
ctxMeanSmoothed = rowMeans(timeseries2matrix(img, ctxMask))
...
```{r adjacency,message=FALSE,warnings=FALSE, fig.width=7, fig.height=5}
