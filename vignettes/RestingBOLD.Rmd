...
sysMatMean = matrix(0, nrow=dim(labeledMat)[1], ncol=nSystems)
sysMatSD = matrix(0, nrow=dim(labeledMat)[1], ncol=nSystems)
...
for ( i in 1:nSystems ) {
  ...
  if ( length(sysIdx) > 0)
    {
    sysMatMean[,i] = rowMeans(labeledMat[,sysIdx])
    sysMatSD[,i] = apply(labeledMat[,sysIdx], 1, sd)
    }
  }
