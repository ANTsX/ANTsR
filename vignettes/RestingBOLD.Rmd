...
```{r frequencyplot,message=FALSE,warnings=FALSE, fig.width=7, fig.height=5}
labels = mask*0
nPts = dim(pts)[1]
for ( i in 1:nPts) {
  pt = as.numeric(pts$x[i], pts$y[i], pts$z[i] )
  idx = antsTransformPhysicalPointToIndex(mean,pt)

  for ( i in c(-n[1]:n[1]) ) {
    for (j in c(-n[2]:n[2])) {
      for (k in c(-n[3]:n[3])) {
        local = idx + c(i,j,k)
        localpt = antsTransformIndexToPhysicalPoint(mean,local)

        dist = sqrt( sum( (localpt-pt)*(localpt-pt) ))
        if ( dist <= r) {
          labels[local[1], local[2], local[3]] = pts$id[i]
         }
        }
      }
    }
