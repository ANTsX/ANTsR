...
refSignal = ( sboldMat %*% t(pr) )
networkDf = data.frame( ROI=refSignal[goodtimes,2],  
...
###### test this for later
refSignal = ( fboldMat %*% t(pr) )
dnz = aslDenoiseR( fboldMat[goodtimes,], refSignal[goodtimes,1], 
  polydegree = 2, crossvalidationgroups=8, verbose=T )
spatmat = t( imageDomainToSpatialMatrix( ctxMask*mask, ctxMask*mask ) )
smoomat = knnSmoothingMatrix( spatmat, k = 100, sigma = 250.0 )
# intensitymat = getNeighborhoodInMask( meanbold, mask, rep(2,3), boundary.condition = 'mean' )
# ismoomat = knnSmoothingMatrix( intensitymat, k = 10, sigma = 100250.0 )
# ismoomat[1000:1002,1000:1002]
image( smoomat[1000:1100,1000:1100])
scl = TRUE
networkDf = data.frame( ROI=scale(refSignal[goodtimes,1], scale=scl ),   
                        scale( dnz$noiseu, scale=scl ) )
test = smoothMatrixPrediction( scale(fboldMat[goodtimes,ctxVox], scale=scl), networkDf, 
  sparsenessQuantile=0.5, smoothingMatrix = smoomat, verbose=T, gamma=1.e-4, iterations=20 )
k=1
print(range(test$v[,k]))
betasI = makeImage( ctxMask*mask, test$v[,k] )
plot( meanbold, betasI, axis=3, nslices=30, ncolumns=10,
        window.overlay = c( 2, max(test$v[,k]) ) )
```
