...
```{r roiplot,message=FALSE,warnings=FALSE, fig.width=7, fig.height=5, echo=FALSE}

means.dat = data.frame(Time=rep( (1:nActualTimes)*tr, nLabels))
yoffset = (rep( 1:nLabels, each=nActualTimes)-1)*0.5
means.dat$Signal = data.frame( Signal=as.vector(roiMat)*yoffset)
#means.dat$ID = rep( 1:nLabels, each=nActualTimes)

#meanPlot = ggplot(means.dat, aes(x=Time, y=Signal, group=ID, colour=ID))
#meanPlot = meanPlot + geom_line(size=0.5)
#meanPlot = meanPlot + theme(text=element_text(size=10), legend.position="top")
