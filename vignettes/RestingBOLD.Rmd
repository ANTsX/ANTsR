...
## Preprocessing
The preprocessing steps include methods used for processing fMRI for
a variety of purposes. This is followed by a section on Connectivity processing
that includes steps specific to the processing of resting state data.
...
globalMean = rowMeans(timeseries2matrix(detrendImg, mask))
...
by the system to which they belong. Connections within a system are color coded while
connections between systems are gray.
...
with an application such as GEPHI
