...
for (ww in unique(comparison_df$w)) {
  ...
  dx2='AD'
  dx1='MCI'
  temp = temp[ temp$diagnosis %in% c(dx1,dx2), ]
  temp$diagnosis_bin = 0
  temp$diagnosis_bin[ temp$diagnosis == dx2 ] = 1
  # 5-fold CV for logistic
  folds <- createFolds(temp$diagnosis_bin, k = 5)
  auc_nsa <- auc_pca <- numeric(5)
...
avg_roc_nsa <- roc(ppmiadni$diagnosis, rowMeans(sapply(roc_nsa_list, function(r) approx(r$specificities, r$sensitivities, xout = seq(0,1,0.01))$y)))
avg_roc_pca <- roc(ppmiadni$diagnosis, rowMeans(sapply(roc_pca_list, function(r) approx(r$specificities, r$sensitivities, xout = seq(0,1,0.01))$y)))
