...
To address these gaps, we propose **Non-negative Stiefel Approximating Flow (NSA-Flow)**, a variational optimization algorithm that refines a non-negative matrix ( $Y \in \mathbb{R}^{p \times k}_{\geq 0}$ ) toward a target ( $X_0$ ) by balancing fidelity, column orthogonality, and non-negativity through a single tunable parameter ( $w \in [0,1]$ ). NSA-Flow rethinks sparsity not as element-wise zeros, but as *global soft orthogonality*—a tunable notion that promotes disjoint supports across columns, fostering interpretable bases. Formulated on the Stiefel manifold [@edelman1998geometry], it employs Riemannian gradient descent [@absil2008optimization] with adaptive momentum, backtracking line search, and flexible retractions (polar, QR, or soft interpolation). Non-negativity is ensured via proximal projections [@parikh2014proximal], maintaining descent stability and constraint satisfaction.

Technically, NSA-Flow operates as a **Riemannian–proximal flow**, combining manifold optimization and proximal projection to enforce multiple structural constraints. Conceptually, it functions as a soft projection operator that can be inserted into any existing machine learning system to improve interpretability—whether as a regularization layer in a neural network [@henaff2011deep; @ricci2025orthogonality], a refinement step in factor models [@li2023unilateral; @rahiche2022variational], or a sparsity-enforcing module in linear embeddings [@guo2019frobenius; @goessmann2020restricted]. Unlike regularization-based methods [@wu2023towards; @kurtz2023group; @ricci2025orthogonality], NSA-Flow operates as a one-sided projection operator, preserving input structure while enabling controlled decorrelation. Compared to orthogonal NMF approaches [@huang2016efficient], it achieves finer tunability without fidelity loss.

NSA-Flow's simplicity and generality make it broadly applicable across domains. It provides a principled and interpretable bridge between sparse PCA, non-negative matrix factorization, and manifold optimization [@edelman1998geometry]; enhances biological and neural data analysis by isolating interpretable components without large fidelity loss [@smith2013functional; @strazar2016orthogonal]; and integrates naturally into existing architectures to enforce controlled sparsity and decorrelation at the whole matrix level. In doing so, NSA-Flow contributes a new lens through which to understand sparsity—not merely as absence, but as a part of global column structure--and a practical mechanism to achieve it across the landscape of interpretable machine learning.
...
2. Rigorous empirical validation, demonstrating superior benchmarks and fidelity compared to state-of-the-art baselines.

3. Broad applications, including enhanced disease classification on the Golub leukemia dataset (15% accuracy gains), non-negative sparse PCA/CCA for biological integration, topic modeling, and interpretable brain network discovery—showcasing NSA-Flow's versatility across ML domains.
...
The defect term is \( f(Y) = \frac{w}{4} \delta(Y) \), scaled by \( c_{\orth} = 4 w / d_0 \), where \( d_0 = \delta(Y_0) \) (floored at \( 10^{-8} \)). With \( n = \|Y\|_F^2 \), \( \hat{G} = Y^\top Y / n \), \( D = \diag(\diag(\hat{G})) \), the gradient is:
...
The combined Euclidean gradient \( \nabla E(Y) = \nabla g(Y) + \nabla f(Y) \) is projected onto the tangent space \( T_Y \mathcal{S}_{p,k} \) [@edelman1998geometry]:

\[
\nabla_\mathcal{M} E(Y) = \nabla E(Y) - Y \sym(Y^\top \nabla E(Y)),
\]

where \( \sym(A) = (A + A^\top)/2 \), ensuring skew-symmetry [@absil2008optimization].


Descent uses \( Z = Y - \alpha \nabla_\mathcal{M} E(Y) \), with adaptive step size \( \alpha \) via backtracking line search (halving \( \alpha \) up to 10 times until energy decreases). An Adam optimizer (\( \beta_1 = 0.9 \), \( \beta_2 = 0.999 \), \( \epsilon = 10^{-8} \)) incorporates momentum [@parikh2014proximal].


Retractions map \( Z \) to the Stiefel manifold [@absil2008optimization]:

- **Polar**: \( R_Y(\Xi) = (Y + \Xi) (I_k + \Xi^\top \Xi)^{-1/2} \).

- **QR**: \( Y + \Xi = QR \), then \( R_Y(\Xi) = Q \sign(\diag(R)) \).

- **Soft QR**: \( T_\omega = (1 - w) I_k + w (Z^\top Z)^{-1/2} \), \( R_Y(\Xi) = Z T_\omega \).
...
1. Relative gradient norm: \( \|\nabla_\mathcal{M} E(Y)\|_F / \|\nabla_\mathcal{M} E(Y_0)\|_F < \tol \).
...
Retraction methods define how an iterative optimization algorithm projects an updated matrix back onto a constraint manifold—in this case, the **orthogonality constraint** (\( \|Y^\top Y - I_k\|_F \approx 0 \)).  
Two key strategies are illustrated here:
...
All retractions optionally preserve the Frobenius norm: scale output \( Y \) by \( \|Y_{\cand}\|_F / \|Y\|_F \) if \( \|Y\|_F > 0 \), focusing optimization on directions.

An optional precondition checks if retraction is needed via the scale-invariant orthogonality defect \( \delta(Y) = \frac{\|Y^\top Y - \diag(\diag(Y^\top Y))\|_F^2}{\|Y\|_F^4} \), computed efficiently as off-diagonal Frobenius squared over norm squared (avoiding temporary matrices). If \( \delta(Y) \leq \tol \cdot \max(1, \|Y\|_F) \), skip for efficiency.

To ensure comparability, input matrices are normalized to unit Frobenius norm before retraction.

We systematically vary \( \omega \) from 0 to 1 and compare how these two retractions affect tall (\( p > k \), e.g., 200 $\times$ 20), wide (\( p < k \), e.g., 20 $\times$ 200), and nearly square (\( p \approx k \), e.g., 100 $\times$ 100) matrices:
...
```{r compare-retractions, fig.width=6, fig.height=9, echo=FALSE, message=FALSE, warning=FALSE}
library(ggplot2)
library(reshape2)
library(patchwork)
...
inv_sqrt_sym <- function(M) {
  e <- eigen(M)
  vals <- pmax(e$values, 1e-12)  # Clip for stability
...
# Updated nsa_flow_retract function
...
#  if (is.null(retraction_type) || retraction_type == "none" || !should_retract(Y_cand, retraction_type) ) return(Y_cand)
  ...
  if (retraction_type %in% c("polar", "svd")) {
    ...
    stop("Unknown retraction_type: ", retraction_type, ". Supported: 'polar', 'soft_polar', 'none'.")
  }
...
run_eval <- function(Z, shape_label) {
  ...
  for (i in seq_along(omegas)) {
    ...
    results$orth_polar[i] <- defect_fast(Y_polar)
    results$fid_polar[i] <- norm(Y_polar - Z, "F")
    
    # Soft Polar
    Y_soft_polar <- nsa_flow_retract(Z, omega, "soft_polar")
    results$orth_soft_polar[i] <- defect_fast(Y_soft_polar)
...
# Color palette
method_colors <- c(
  "polar" = "#1f78b4", "soft_polar" = "#6a3d9a"
)
...
p1 <- ggplot(subset(df_melt, grepl("orth_", variable)),
  ...
  labs(title = "Orthogonality Defect vs \u03C9",
       y = "||Y\u1D40Y - diag(Y\u1D40Y)||\u00B2 / ||Y||_F^4", x = "Interpolation weight (\u03C9)", color = "Method") +
  facet_wrap(~ shape) +
...
p2 <- ggplot(subset(df_melt, grepl("fid_", variable)),
  ...
  labs(title = "Fidelity (Deviation from Z) vs \u03C9",
       y = "||Y - Z||_F", x = "Interpolation weight (\u03C9)", color = "Method") +
  facet_wrap(~ shape) +
  theme_minimal(base_size = 14)
...
set.seed(123)
p <- 20; k <- 10
X0 = generate_synth_data( p, k, corrval=0.3, noise=0.5, sparse_prob=0.0, include_neg=TRUE )$Y0
```


```{r stiefel_sweep_full, fig.width=8, fig.height=5.3, echo=FALSE, message=FALSE}
###
...
for(i in seq_along(w_seq)) {
  ...
  mats[[i]] <- res_soft_w$Y
  convergeplots[[i]] <- res_soft_w$plot+labs(title = paste0("w = ", round(w_val, 3)), subtitle = NULL)
}
...
ogtit = paste0("Original ",', orth = ', 
    ...
    1.0-round(sum(X0/max(X0) > 0.01)/length(X0),3))
plots[[1]] <- pheatmap(X0[rows_show,], cluster_rows = FALSE, cluster_cols = FALSE, color = cols, main = ogtit, silent = TRUE, breaks = seq(rng[1], rng[2], length.out = 121))
...
grid.arrange(grobs=convergeplots, top='Convergence Plots for Different w Values', ncol=2 )
#----------#
```
...
k_basis <- 10        # Number of basis components
...
if ( ! exists("sens_metrics")) {
    ...
    res <- sparse_pca_imp(golub_scaled_ss, k = myk,
        lambda = 0.1, alpha = 0.001, max_iter = mxit,
        proximal_type = "nsa_flow", retraction='soft_polar',
        tol = 1e-5, w = omega, verbose = TRUE)
...
# List of optimizers
myopts <- 'armijo_gradient' # list_simlr_optimizers()

# Weights to test (uncomment full list if needed)
wws <- runif( 100, 0.01, 0.99)
...
```{r beautiful-viz, fig.height=3.5, fig.width=7}
library(ggplot2)
