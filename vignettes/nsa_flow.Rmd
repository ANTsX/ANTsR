...
Algorithm: NSA-Flow
Input:
  Y_0: Initial matrix (p × k)
  X_0: Target matrix (p × k, optional, default: pmax(Y_0, 0) + perturbation)
...
Retraction methods define how an iterative optimization algorithm projects an updated matrix back onto a constraint manifold—in this case, the **orthogonality constraint** (‖YᵀY − I‖ ≈ 0).  
...
- **Orthogonality Defect**:  
  \( \|Y^\top Y - \mathrm{diag}(Y^\top Y)\|_F^2 \) — measures deviation from column orthogonality.  
- **Fidelity**:  
  \( \|Y - Z\|_F \) — measures deviation from the input update.  
- **Timing**:  
  Average computation time for each retraction method.  
- **Singular Value Spectrum**:  
  Distribution of singular values of the resulting matrices, showing how “balanced” each method keeps the column norms.
...
p1 <- ggplot(subset(df_melt, grepl("orth_", variable)),
  ...
  labs(title = "Orthogonality Defect vs w",
       y = "‖YᵀY − diag(YᵀY)‖²", x = "Interpolation weight (w)", color = "Method") +
...
p2 <- ggplot(subset(df_melt, grepl("fid_", variable)),
  ...
  labs(title = "Fidelity (Deviation from Z) vs w",
       y = "‖Y − Z‖ₙₒᵣₘ", x = "Interpolation weight (w)", color = "Method") +
  theme_minimal(base_size = 14)
...
def_ret = "soft"
nsa_default <- function(Y0, w, verbose = FALSE ) {
  nsa_flow(
    Y0 = Y0,
    X0 = NULL,
    w = w,
...
res_soft <- nsa_flow(X0,  w = w, retraction = "soft", plot=TRUE, max_iter = mxit, seed = 1, record_every = 1, initial_learning_rate = ini_default, verbose = FALSE )
...
compute_core_metrics <- function(Y, X_or_S) {
  ...
  if (nrow(X_or_S) == ncol(X_or_S)) {
    S <- X_or_S                # it's a covariance matrix (p × p)
  } else {
    S <- cov(X_or_S)           # it's raw data (n × p)
  }
...
Figure 8 shows the top 5 terms for selected topics, highlighting NSA-Flow’s ability to produce distinct, interpretable topics.
...
```{r brainexample,echo=FALSE, fig.width=10, fig.height=6, message=FALSE, warning=FALSE,eval=FALSE}
library(ANTsR)
library(NMF) # For NMF initialization
library(ggplot2)
library(reshape2) # For melting data (though using tidyr now)
library(dplyr)
library(tidyr) # For pivot_longer
library(fmsb) # For radarchart (install if needed: install.packages("fmsb"))
library(igraph) # For graph visualization
library(scales) # For alpha in radarchart
library(ggpubr)
set.seed(42)

# Assume nsa_flow is defined in the environment from previous refactoring

# Step 1: Generate synthetic cortical thickness data using DKT atlas
# 200 subjects (rows), 76 brain regions (columns)
N <- 200 # subjects
p <- 76 # regions
# Generate non-negative data simulating cortical thickness (mean ~2.5 mm, sd ~0.5 mm, clipped to >=0)
X_data <- matrix(rnorm(N * p, mean = 2.5, sd = 0.5), nrow = N, ncol = p)
X_data[X_data < 0] <- 0
# Reasonable brain region names based on Desikan-Killiany (DK) atlas: 34 cortical per hemisphere (68) + 4 subcortical per hemisphere (8) = 76
# Note: DKT is an extension, but DK is core; using standard DK labels
cortical_regions <- c(
...
# Add 4 subcortical per hemisphere
subcortical <- c('thalamus', 'caudate', 'putamen', 'hippocampus')
regions <- c(regions, paste0('left_', subcortical), paste0('right_', subcortical))
stopifnot(length(regions) == 76)
# Create data.frame for interpretability
X <- as.data.frame(X_data)
colnames(X) <- regions
cat("Sample of generated data:\n")
print(head(X[, 1:5])) # Show first few columns
w=0.999
M_nsa <- nsa_flow( t(data.matrix(X)), w = w, retraction = 'soft_svd', 
    initial_learning_rate=10,
    max_iter=500, verbose=T)$Y
invariant_orthogonality_defect(M_nsa)
mvec=as.numeric( t(M_nsa)  )
mvecsel=mvec>quantile(mvec[mvec>0],0.2)
scatdf=data.frame( raw_vals=as.numeric(data.matrix(X))[mvecsel], nsa_vals=as.numeric(mvec[mvecsel]) )
ggscatter( scatdf, x = "raw_vals", y = "nsa_vals",
           add = "reg.line", conf.int = TRUE,
           cor.coef = TRUE, cor.method = "pearson",
           xlab = "Raw Cortical Thickness", ylab = "NSA-Flow Reconstructed Values",
           title = "NSA-Flow Reconstruction of Cortical Thickness Data",
           ggtheme = theme_minimal(base_size = 14) ) +
  theme(plot.title = element_text(hjust = 0.5, face = "bold"))

M_nsa <- nsa_flow( t( X ), w = w, retraction = 'soft_svd', apply_nonneg = TRUE)$Y
rownames(M_nsa) <- regions
# Normalize columns to sum to 1 for interpretability (relative loadings per network)
Y_norm <- sweep(M_nsa, 2, colSums(M_nsa), "/")

# 4.1: Heatmap of entire basis matrix Y (regions x networks) - elegant with ggplot
Y_df <- as.data.frame(Y_norm) %>%
  rownames_to_column(var = "Region") %>%
  pivot_longer(cols = -Region, names_to = "Network", values_to = "Loading")
ggplot(Y_df, aes(x = Network, y = Region, fill = Loading)) +
  geom_tile(color = "white", size = 0.1) +
  scale_fill_viridis_c(option = "plasma", direction = -1) +
  theme_minimal(base_size = 12) +
  labs(title = "Network Loadings Across Brain Regions",
       x = "Networks", y = "Brain Regions", fill = "Loading") +
  theme(axis.text.y = element_text(size = 6), # Small text for many regions
        plot.title = element_text(hjust = 0.5, face = "bold"),
        panel.grid = element_blank()) +
  coord_fixed(ratio = 0.2) # Adjust aspect for readability

# 4.2: For each network, horizontal bar plot of top 10 regions - sleek and colorful
for (net in 1:k) {
  loadings <- na.omit(sort(Y_norm[, net], decreasing = TRUE)[1:10])
  load_df <- data.frame(Region = names(loadings), Loading = loadings) %>%
    mutate(Region = factor(Region, levels = Region))
  ggplot(load_df, aes(x = Loading, y = Region, fill = Loading)) +
    geom_bar(stat = "identity", width = 0.7) +
    scale_fill_gradient2(low = "lightblue", high = "darkblue", midpoint = median(load_df$Loading)) +
    theme_minimal(base_size = 14) +
    labs(title = paste("Top Regions in Network", net),
         x = "Loading", y = "") +
    theme(plot.title = element_text(hjust = 0.5, face = "bold"),
          axis.text.y = element_text(size = 12, color = "black"),
          panel.grid.major.y = element_blank()) +
    geom_text(aes(label = round(Loading, 2)), hjust = -0.1, size = 4)
  # ggsave(paste0("top_regions_network_", net, ".png"), width = 12, height = 6)
}

# 4.3: Radar plot for network profiles - aesthetic radial visualization with fmsb (top 10 regions only, to avoid overcrowding)
for (net in 1:k) {
  loadings <- Y_norm[, net]
  top_idx <- order(loadings, decreasing = TRUE)[1:10]
  loadings_top <- loadings[top_idx]
  regions_top <- names(loadings_top)
  # For radarchart, need data.frame with max/min rows for scaling
  radar_df <- rbind(rep(max(loadings_top) * 1.1, 10), rep(0, 10), loadings_top)
  colnames(radar_df) <- regions_top
  radarchart(radar_df, axistype = 1,
             pcol = "teal", pfcol = alpha("teal", 0.25), plwd = 2,
             cglcol = "grey", cglty = 1, axislabcol = "grey", caxislabels = seq(0, max(loadings_top), length = 5),
             cglwd = 0.8, vlcex = 0.8) # Adjusted for 10 labels
  title(paste("Radial Profile of Network", net, "(Top 10 Regions)"), line = 3)
  # Save as PNG if needed: dev.print(png, filename = paste0("radar_network_", net, ".png"), width = 800, height = 800)
}

# 4.4: Network graph visualization - beautiful connectome-like view with igraph
# Improved lobe mapping (for demo; assign regions to lobes)
lobe_map <- sapply(regions, function(r) {
  reg <- substr(r, 6, nchar(r))  # Remove 'left_' or 'right_'
  if (grepl("bankssts|caudalmiddlefrontal|rostralmiddlefrontal|superiorfrontal|frontalpole|parsopercularis|parsorbitalis|parstriangularis|lateralorbitofrontal|medialorbitofrontal", reg)) "frontal" else
  if (grepl("inferiortemporal|middletemporal|superiortemporal|temporalpole|transversetemporal|fusiform|entorhinal|parahippocampal", reg)) "temporal" else
  if (grepl("inferiorparietal|superiorparietal|supramarginal|precuneus|postcentral|paracentral", reg)) "parietal" else
  if (grepl("lateraloccipital|cuneus|pericalcarine|lingual", reg)) "occipital" else
  if (grepl("caudalanteriorcingulate|isthmuscingulate|posteriorcingulate|rostralanteriorcingulate|insula", reg)) "limbic" else
  if (grepl("hippocampus", reg)) "limbic" else "subcortical"
})
lobe_colors <- c(frontal = "blue", temporal = "green", parietal = "red",
                 occipital = "purple", limbic = "orange", subcortical = "grey")
for (net in 1:k) {
  threshold <- quantile(Y_norm[, net], 0.8) # Top 20% regions
  ...
  V(g)$color <- lobe_colors[V(g)$lobe]
  V(g)$size <- V(g)$loading * 30 # Size by loading
  # Add edges between regions in same lobe (demo; in real, use connectivity matrix)
  lobe_groups <- split(seq_along(active_regions), lobes_active)
  for (group_name in names(lobe_groups)) {
    group <- lobe_groups[[group_name]]
    if (length(group) > 1) {
      comb_pairs <- t(combn(group, 2))
      g <- add_edges(g, comb_pairs)
    }
  }
  # Layout and plot
  ...
  plot(g, layout = layout, vertex.label = V(g)$name, vertex.label.cex = 0.7,
       ...
       main = paste("Graph Representation of Network", net))
  # Add legend for lobes
  legend("topright", legend = names(lobe_colors), fill = lobe_colors, title = "Lobes", cex = 0.8)
  # Save: dev.print(png, filename = paste0("graph_network_", net, ".png"), width = 1200, height = 1200)
}
