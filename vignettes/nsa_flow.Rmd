...
Ymat <- M_nsa$Y
# add predictors to 
proj = data.matrix(X) %*% data.matrix(Ymat)
colnames(proj) = paste0("nsa_net",1:ncol(proj))
ppmiadni=cbind(ppmiadni, proj)
for ( cog in adni_variables ) {
    covars=c("AGE", "PTGENDER", "PTEDUCAT", "APOE4")
    covarsf=paste(covars, collapse="+")
    myform=paste(cog, "~", covarsf, '+', paste(colnames(proj), collapse="+") )
    temp=ppmiadni[, c(cog, covars, colnames(proj)) ]
    mod=lm( myform  , data=temp )    
    print( summary(mod) )
    cat("cog:", cog, "\n" )
    Sys.sleep(3)
}
