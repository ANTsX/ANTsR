...
We consider the problem of learning or adapting an orthonormal basis matrix  \( Y \in \mathbb{R}^{p \times k} \) to approximate a target \( X_0 \in \mathbb{R}^{p \times k} \) while enforcing column orthogonality and non-negativity. This formulation is common in orthogonal factorization, manifold learning, and related optimization problems[@parikh2014proximal; @absil2008optimization; @edelman1998geometry].  The NSA-Flow optimization problem is defined as

\[
\min_{Y \in \mathbb{R}^{p \times k}} 
E(Y) 
= (1 - w) \, \| Y - X_0 \|_F^2 
+ w \, \text{Orth}(Y),
\]
where \( 0 \leq w \leq 1 \) is a weight parameter trading off fidelity to the target
against column orthogonality. The orthogonality penalty is given by
\[
\text{Orth}(Y) \;=\; \frac{1}{2} \, \| Y^\top Y - I_k \|_F^2,
\]
which vanishes when the columns of \( Y \) are orthonormal and grows quadratically
with orthogonality defect.

The gradient of the orthogonality term is
\[
\nabla_Y \text{Orth}(Y) \;=\; Y \, ( Y^\top Y - I_k ),
\]
and the total gradient of the energy is
\[
\nabla_Y E(Y) \;=\; 2 (1 - w)(Y - X_0) \;+\; w \, Y ( Y^\top Y - I_k ).
\]
...
Y \leftarrow Y - \eta \, \nabla_Y E(Y)
\]
does not preserve orthogonality and typically requires a retraction step or alternative stabilization.  NSA-Flow uses a **soft retraction-free (soft-RF)** update rule that interpolates between the raw gradient descent iterate and its polar retraction:

\[
Y^{(t+1)} \;=\; (1 - \lambda) \, \widetilde{Y}^{(t+1)} \;+\; 
\lambda \, \mathrm{Retract} \bigl( \widetilde{Y}^{(t+1)} \bigr),
\]
with \( 0 \leq \lambda \leq 1 \) and
\(
\widetilde{Y}^{(t+1)} = Y^{(t)} - \eta \, \nabla_Y E(Y^{(t)}).
\).

For \( \lambda = 1 \), the method recovers a full polar/SVD retraction step. For \( \lambda = 0 \), the update is purely Euclidean.  Intermediate values provide a **low-cost, stable correction** that reduces orthogonality defect without performing an expensive retraction every iteration [@vary2024optimization; @ablin2022fast].

### Pseudo-Proof of Stability (Soft-RF)

**Claim.**  
Let \( \widetilde{Y} = Y - \eta \nabla_Y E(Y) \) and \( Q = \mathrm{Retract}(\widetilde{Y}) \).
Then for \( 0 \leq \lambda \leq 1 \),
\[
\| Y^{\text{new}\,\top} Y^{\text{new}} - I \|_F
\;\leq\;
(1 - \lambda) \| \widetilde{Y}^\top \widetilde{Y} - I \|_F.
\]

**Sketch of proof.**  
The polar retraction minimizes the Frobenius distance to the Stiefel manifold. By convexity of the Frobenius norm and linearity of interpolation,
\(
\| (1-\lambda)\widetilde{Y} + \lambda Q \|_F
\)
cannot increase the orthogonality defect relative to the unretracted iterate. Since \( Q \) lies on the manifold, the inequality follows directly. This guarantees strict decrease in defect for any \( \lambda > 0 \).

To stabilize optimization across scales, NSA-Flow optionally employs a **scale-invariant orthogonality defect** defined as

\[
\mathrm{Defect}(Y) \;=\; 
\frac{\| Y^\top Y - I \|_F}{\max(1, \|Y\|_F)}.
\]

This normalization prevents large norms from dominating the penalty, allowing robust step-size selection and more consistent convergence [@liu2021quadratic; @song2025distributed].

### Cayley Transform (Future Work)

The Cayley transform defines an **exact retraction-free** orthogonal update:

\[
Y^{(t+1)} \;=\; \bigl( I - \tfrac{\eta}{2} A \bigr)^{-1}
\bigl( I + \tfrac{\eta}{2} A \bigr) Y^{(t)},
\]
where \( A \) is a skew-symmetric matrix constructed from the gradient.
This update preserves orthogonality exactly but requires solving linear systems
at each iteration. We regard scalable low-rank Cayley implementations
as promising future work.

### Computational Complexity

We summarize the **per-iteration complexity** of different update mechanisms:

| Method                 | Dominant Operation                    | Complexity                                 | Orthogonality |
|-------------------------|---------------------------------------|---------------------------------------------|---------------|
| Euclidean GD (none)     | Gradient + matmul                     | \( O(p k^2) \)                              | ✗             |
| Polar Retraction        | SVD or polar decomposition            | \( O(\min(p,k) k^2) \) to \( O(p k^2) \)     | ✓ (exact)     |
| Soft-RF (ours)          | Matmul + partial retraction           | \( O(p k^2) \) (reduced constant factor)    | ≈ (controlled)|
| Cayley Transform        | Linear solve (low rank)               | \( O(p k^2) \) to \( O(k^3) \)              | ✓ (exact)     |

- For **tall-skinny** matrices (\( p \gg k \)), soft-RF avoids unnecessary work on the null space and outperforms full retraction.

- For **square** cases (\( p \approx k \)), SVD and Cayley have similar asymptotics, but soft-RF retains lower constants.

- For **wide** matrices (\( k \gg p \)), soft-RF maintains efficiency via fast matmul and structured updates.

Overall, soft-RF achieves stable convergence at significantly reduced computational cost compared to full retraction, with tunable defect control via \( \lambda \).
...
The following flowchart visualizes the NSA-Flow algorithm's workflow, highlighting the iterative process, retraction choices, and convergence checks.
...
```{r enhanced-stats-dx,echo=FALSE,fig.width=7,fig.height=3}
