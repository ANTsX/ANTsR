...
sparse_pca_imp <- function(X, k, lambda = 0.1, alpha = 0.0001, 
  ...
  if (w_orth < 0 || w_orth > 1) stop("w_orth must be in [0,1]")
  ...
  for (iter in seq_len(max_iter)) {
    ...
    grad_o <- matrix(0, p, k)
    eu_grad <- w_orth * grad_o + w_pca * grad_p
    ...
    if (w_orth > 0) {
      sym_ytg <- ANTsR::symm(t(Y) %*% eu_grad)
      rgrad <- eu_grad - Y %*% sym_ytg
    } else {
      rgrad <- eu_grad
    }
    ...
    energy_of <- function(M) {
      ...
      orth_term <- if (w_orth > 0) w_orth * orth_residual(M) else 0
    ...
    if (proximal_type == "basic") {
      ...
      Y_temp <- if (apply_soft_thresh_in_nns) soft_threshold(Y_ret, thresh) else Y_ret
      prox_res <- nsa_default(Y_temp,  w = w )
      Y_new <- prox_res$Y
    }
    ...
    orth_e <- if (w_orth > 0) orth_residual(Y_new) else 0
    tr_val <- sum(diag(t(Y_new) %*% t(Xc) %*% (Xc %*% Y_new))) / n
    expl_var_ratio <- min(tr_val / total_var, 1)
    ...
    if (verbose) {
      cat(sprintf("Iter %3d | Energy: %12.6e | Orth: %.4e | Sparsity: %.4e | ExplVar: %.4f | rgrad_norm: %.4e | bt: %2d | alpha: %.3e | t: %.2fs\n",
                  iter, energy, orth_e, sparsity_level(Y_new), expl_var_ratio, rgrad_norm, bt, alpha, as.numeric(Sys.time() - t_start, units="secs")))
...
library(topicmodels); library(tm); library(NMF)
set.seed(42)
# Load AssociatedPress dataset (mocked here; use actual dataset in practice)
data("AssociatedPress", package = "topicmodels")
term_doc_matrix <- as.matrix(AssociatedPress)  # Terms x Documents
term_doc_matrix 
p <- nrow(term_doc_matrix); n <- ncol(term_doc_matrix)
# NSA-Flow
Y0 <- matrix(runif(p*k, 0, 1), p, k)  # Random initialization
res_nsa <- nsa_default(term_doc_matrix, w = DEFAULT_W )
nsa_topics <- res_nsa$Y
k=3
# Standard NMF
nmf_fit <- nmf(term_doc_matrix, rank = k, method = "Frobenius")
...
res_basic <- sparse_pca_imp(golub_scaled_ss, k = myk,
    ...
    proximal_type = "basic",
    w_orth = 0.5, tol = 1e-5, w = 0.5, verbose = FALSE)
...
```{r pca_sens_metrics, echo=FALSE, fig.width=13, fig.height=6, message=FALSE, warning=FALSE,eval=FALSE}
# Sensitivity Sweep (Ω)
omega_vals <- seq(0, 0.5, by = 0.1)
n <- nrow(golub_scaled_ss)
A <- t(golub_scaled_ss) %*% golub_scaled_ss / n
sens_results <- lapply(omega_vals, function(omega) {
    print( omega)
  res <- sparse_pca_imp(
    golub_scaled_ss, k = myk,
    lambda = 0.1, alpha = 0.01, max_iter = 500,
    proximal_type = "nsa_flow",
    w_orth = 1., tol = 1e-2, w = omega, verbose = T
  )
  proj <- golub_scaled_ss %*% res$Y
  acc_res <- cv_acc(proj, labels)
  tr_val <- sum(diag(t(res$Y) %*% A %*% res$Y))
  total_var <- sum(diag(A))
  expl_var_ratio <- min(tr_val / total_var, 1)
  data.frame(
    Omega = omega,
    Accuracy = acc_res$Accuracy,
    Explained_Var_Ratio = expl_var_ratio,
...
sens_long <- sens_metrics %>%
  pivot_longer(cols = c(Accuracy, Explained_Var_Ratio, Sparsity, Orth_Residual),
...
**Interpretation**: On this real high-dimensional dataset, our sparse PCA variants achieve high explained variance with substantial sparsity (>90% zeros), selecting a small number of genes while maintaining near-orthogonality. The nsa_flow variant enforces non-negativity, leading to interpretable positive loadings and slightly better classification accuracy due to reduced noise. Visualizations show clear ALL/AML separation in 2D and 3D, comparable to standard PCA but with far fewer genes—highlighting practical value for biomarker identification in oncology. 
