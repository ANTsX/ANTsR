...
# t-SNE visualization
tsne_result <- Rtsne(data_X, dims = 2, perplexity = min(30, n_samples/4), verbose = TRUE, max_iter = 500)
tsne_df <- data.frame(
...
for (ww in seq(0.05, 0.95, by = 0.05)) {
  ...
  for (f in 1:nfold) {
    ...
    cat("Fold:", f, "Weight:", ww, "Classes in test$diagnosis:", levels(test$diagnosis)[table(test$diagnosis) > 0], "\n")
