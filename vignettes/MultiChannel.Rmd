...
The `lapply` function provides a convenient way to process all channels. Here it
is worth mentioning that ANTsR is not yet compatible with `mclapply` as provided
in the "parallel" package. It's something we will be looking into in the future.
...
eigs = unlist( apply(mat, 2, function(x) { eigen( initTensor(x) ) } ) )
