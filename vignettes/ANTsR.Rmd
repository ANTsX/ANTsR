...
```{r basic plot,message=FALSE,warnings=FALSE,echo=FALSE}
...
segview<-as.antsImage( rbind(as.array(iMath(fi,"Normalize")),
  as.array(iMath(seg$segmentation,"Normalize"))))
invisible(plot(segview))
```
...
regresult<-as.antsImage( rbind( as.array(iMath(mytx$warpedmovout,"Normalize")),
  as.array(iMath(fi,"Normalize")) ) )
invisible(plot(regresult))
```
...
neighborhood information.  Image neighborhoods contain rich shape and texture.
Here, we extract neighborhoods for further analysis at a given scale.
```{r nhood}
...
or the image boundaries.  This example replaces missing data with the mean
...
also get this data from `antsPreprocessfMRI` which also provides denoising
...
```{r eanat,echo=TRUE,message=FALSE,warning=FALSE}
