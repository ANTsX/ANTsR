...
## Basics
Here we illustrate how an Affine transform may be created and applied to point and images. When applying
...
point = c(80,40)
antsApplyTransformToPoint(tx, point)

pts = list(c(80,40), c(20,30))
lapply( pts, function(x) {antsApplyTransformToPoint(tx, x)})
...
Methods for writing transforms to file are under development.
...
#warpTx = antsTransformFromDisplacementField(antsImageRead("yourfield.nii.gz"))

```

## Composing Transforms
Transforms may be composed by creating a list of transforms in the reverse order that they
should be applied
