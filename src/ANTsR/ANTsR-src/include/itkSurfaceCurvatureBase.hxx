/*=========================================================================
  ...
  Module:    $RCSfile: itkSurfaceCurvatureBase.hxx,v $
  Language:  C++
...
//  dim=2;
  ...
  for (unsigned int i =0; i<npts; i++){
    for(unsigned int j = 0; j < dim; j++) {
    for(unsigned int k = 0; k < dim; k++) {
      D(j,k) +=  m_TangentProjectionList[i][j]*m_TangentProjectionList[i][k]*m_WeightVector[i]*(m_DirectionalKappaVector[i]);//
    }}
  }
...
::CharacterizeSurface()
{
  ...
  if      (m_MeanKappa > 0 && m_GaussianKappa > 0 ) return 1; // bowl
  else if (m_MeanKappa < 0 && m_GaussianKappa > 0 ) return 2; // inv bowl
  else if (m_MeanKappa > 0 && m_GaussianKappa <  0) return 3; // pos saddle 
  else if (m_MeanKappa < 0 && m_GaussianKappa <  0) return 4; // neg saddle 
  else if (m_MeanKappa > 0 && m_GaussianKappa == 0) return 5; // pos U 
  else if (m_MeanKappa < 0 && m_GaussianKappa == 0) return 6; // neg U
  else if (m_MeanKappa ==0 && m_GaussianKappa == 0) return 7; // flat
  else if (m_MeanKappa ==0 && m_GaussianKappa <  0) return 8; // perfect saddle 
  else return 0;
...
// old below
  ...
  else if (m_MeanKappa ==0 && m_GaussianKappa == 0) return 5;
  else if (m_MeanKappa ==0 && m_GaussianKappa <  0) return 4;
  else if (m_MeanKappa < 0 && m_GaussianKappa >  0) return 3;
  else if (m_MeanKappa < 0 && m_GaussianKappa == 0) return 2;
  else if (m_MeanKappa < 0 && m_GaussianKappa <  0) return 1;
  else return 0;
...
void  SurfaceCurvatureBase<TSurface,TDimension>::TestEstimateTangentPlane(PointType origin)
{
 // Read points from stdin
  ...
  for(unsigned int i = 0; i < npts; i++) {
    for(unsigned int j = 0; j < dim; j++) D(i,j) = (pts(i,j)-origin(j))*(pts(i,j)-origin(j));
    //D(i,dim) = 1;
  }
...
/*
  ...
  if (fabs(denom) > 1.e-12) m_Kappa2=(m_Eval2-m_Eval1*w3/w1)/denom;
  else 
  {
...
::EstimateMetricTensor()
{
   ...
   float a=0;
   for (unsigned int i=0; i<3;i++) a+=m_Tangent1[i]*m_Tangent1[i];
   m_MetricTensor[0]=a;
   
   float b=0;
   for (unsigned int i=0; i<3;i++) b+=m_Tangent2[i]*m_Tangent1[i];
   m_MetricTensor[1]=b;
   
   float c=0;
   for (unsigned int i=0; i<3;i++) c+=m_Tangent2[i]*m_Tangent2[i];
   m_MetricTensor[2]=c;
...
// Build cov matrix D
  ...
  float mx;
  if (meanu1>meanu2) mx=meanu1; else mx=meanu2;
  meanu1/=mx;meanu2/=mx;
