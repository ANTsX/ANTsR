...
protected:
  ...
  virtual void GenerateData( void )
    ...
    bool verbose=true;
    if (verbose) cout << endl << "   converting image to VTK" << endl;
    fltImport->Update();
    ...
    if(m_DecimateFactor > 0.0  )
      {
      if (verbose) cout << "   decimating the mesh by factor of " << m_DecimateFactor << endl;
    ...
    if(m_SmoothingIterations > 0  )
      {
      if (verbose) cout << "   smoothing the mesh " << m_SmoothingIterations  << endl;
