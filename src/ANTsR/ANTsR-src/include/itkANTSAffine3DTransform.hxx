...
//     Rcpp::Rcout << "compute para: to be done!" << std::endl;
...
//     R = dq * R;
    ...
    Rcpp::Rcout << "rotation R" << Q << std::endl;
    Rcpp::Rcout << "upper R" << R << std::endl;
    Rcpp::Rcout << "s=" << s << " u=" << u << " v=" << v << " w" << w << " r="
            << r << std::endl;

    m_Rotation = VnlQuaternionType(u, v, w, r);

    Rcpp::Rcout << "m_Rotation from vnl" << VnlQuaternionType(u, v, w, r)
            << std::endl;
    ...
    // Rcpp::Rcout << "before: this->GetMatrix(): " << this->GetMatrix();

    this->ComputeMatrix();

    // Rcpp::Rcout << "after: this->GetMatrix(): " << this->GetMatrix();
