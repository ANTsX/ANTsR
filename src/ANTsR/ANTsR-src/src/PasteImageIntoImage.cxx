...
RcppExport SEXP PasteImageIntoImage( SEXP r_args )
try
{
  ...
  if ( argc < 6 )
    {
    Rcpp::Rcout << argv[0] << " imageDimension inputCanvasImage inputImage "
      << "outputImage startIndex [backgroundLabel=0] [paintOverNonBackgroundVoxels=0] [conflictLabel=-1]" << std::endl;
    Rcpp::Rcout << "   If the current painting image voxel is nonbackground and corresponds to a background voxel in the canvas image "<< std::endl;
    Rcpp::Rcout << "     paintOverNonBackgroundVoxels = 0 -> leave the canvas voxel as is." << std::endl;
    Rcpp::Rcout << "     paintOverNonBackgroundVoxels = 1 -> replace canvas voxel value with painting image voxel value"  << std::endl;
    Rcpp::Rcout << "     paintOverNonBackgroundVoxels = 2 -> replace canvas voxel walue with conflictLabel"  << std::endl;
