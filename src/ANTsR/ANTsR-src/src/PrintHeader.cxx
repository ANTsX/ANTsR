...
int PrintHeader(int argc, char *argv[])        
{
  ...
  if ( ImageDimension == 1 ) 
    Rcpp::Rcout << " Size : " << reader->GetOutput()->GetLargestPossibleRegion().GetSize()[0] << " " <<   std::endl;
  else if ( ImageDimension == 2 )
    Rcpp::Rcout << " Size : " << reader->GetOutput()->GetLargestPossibleRegion().GetSize()[0] << " " << reader->GetOutput()->GetLargestPossibleRegion().GetSize()[1] << " " << std::endl;  
   else if ( ImageDimension == 3 )
    Rcpp::Rcout << " Size : " << reader->GetOutput()->GetLargestPossibleRegion().GetSize()[0] << " " << reader->GetOutput()->GetLargestPossibleRegion().GetSize()[1] << " " <<  " " << reader->GetOutput()->GetLargestPossibleRegion().GetSize()[2] << std::endl;
   else    Rcpp::Rcout << " Size : " << reader->GetOutput()->GetLargestPossibleRegion().GetSize() << std::endl;
//  Rcpp::Rcout << " Orientation " << reader->GetOutput()->GetOrientation() << std::endl;
  ...
  if(!full) 
    ...
    Rcpp::Rcout << image->GetDirection().GetVnlMatrix() << std::endl;
    ...
    //    Rcpp::Rcout << image->GetVoxelSpaceToRASPhysicalSpaceMatrix().GetVnlMatrix() << std::endl;
...
try
{
  ...
  if ( argc < 2  || ((argc == 2) && strcmp(argv[1], "--help") == 0))
    { Rcpp::Rcout << "Usage:  " << argv[0] << " image.ext " << std::endl;
    return Rcpp::wrap( EXIT_FAILURE ) ;
  }           
   // Get the image dimension
  std::string fn = std::string(argv[1]);
 if ( ! FileExists(fn) ) { Rcpp::Rcout << " file " << fn << " does not exist . " << std::endl;  return Rcpp::wrap( EXIT_FAILURE ) ; } 
  itk::ImageIOBase::Pointer imageIO =
   ...
   {
     Rcpp::Rcout << " cant read " << fn << std::endl;
     return Rcpp::wrap( EXIT_FAILURE ) ;
   }
   ...
   case 4:
     PrintHeader<4>(argc,argv);
      break;
   default:
      Rcpp::Rcout << "Unsupported dimension " <<  imageIO->GetNumberOfDimensions() << std::endl;
      return Rcpp::wrap( EXIT_FAILURE ) ;
   }
