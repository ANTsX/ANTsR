...
{
  //  Rcpp::Rcout << " reading b " << std::string(file) << std::endl;
...
{
  Rcpp::Rcout << " MATCHING INTENSITIES " << std::endl;
	       ...
	       std::string outfn, bool TRUTH)
{
  ...
  */
  ...
  //  for (int i=0; i<=maximum; i++) 
  //  Rcpp::Rcout << " label " << i << " ct is: " << histogram[i] << std::endl;
...
float npdf(std::vector<float> vec, bool opt,  float www) {
		...
		//	else Rcpp::Rcout << " Mean " << mean << " var " << var << std::endl;
...
//		float sample=0.0;
		...
		//		Rcpp::Rcout << " using width " << width << std::endl;
//		float N=(float)size;
		...
		for (unsigned int i=0; i<size; i++)
		  {
		    ...
		    if (simval > maxsim) { maxsim=simval; max=i; }
		    //		    Rcpp::Rcout << " simval " << simval << " i " << i << " appval " << appval << " maxsim " << maxsim << " max " << max << std::endl;
		  }
		//		Rcpp::Rcout <<"  estapp " << estapp << " max " << max << std::endl;
		//return estapp; 
		if ( opt == true )
		return labelvec[max];      
		else 
...
int ImageSetStatistics(int argc, char *argv[])        
{
  ...
  //  Rcpp::Rcout <<" roifn " << roifn << " fn1 " << fn1 << " whichstat " << whichstat << std::endl;
  ...
  if (roifn.length() > 4)
    {
      Rcpp::Rcout <<" reading roi image " << roifn << std::endl;
	...
	{
	  ROIimg=NULL;
	  Rcpp::Rcout << " Error reading ROI image " << std::endl;
	  //  return 0;
	} 
  ...
  if ( !inputStreamA.is_open() )
    {
      Rcpp::Rcout << "Can't open parameter file: " << fn1 << std::endl;  
      return -1;
    }
  while ( !inputStreamA.eof() )
	...
	{
	  //	  Rcpp::Rcout << "Done.  read " << lineBuffer << " n " << ct1 << " files " << std::endl;
	  //Rcpp::Rcout << std::endl;
	  continue;
	}
      else
  ...
  Rcpp::Rcout << " NFiles1 " << filecount1 << std::endl;
  ...
  if ( filecount1 != filecount2 ) 
    {
      Rcpp::Rcout << " the number of similarity images does not match the number of label images --- thus, we have to get out of here !! i.e. something's wrong. " << std::endl;
      return 1; 
    }
  } // fi simimagelist 
  Rcpp::Rcout << " NFiles2 " << filecount2 << std::endl;
  ...
  while ( !inputStreamA.eof() )
      ...
      else
	{
	  ...
	  Rcpp::Rcout << " done reading " << (float) ct / (float ) filecount1 << std::endl;
	  ct++;
	}
    }
  inputStreamA.close(); 
  ...
  ct=0;
      ...
      for(  vfIter.GoToBegin(); !vfIter.IsAtEnd(); ++vfIter )
	{
	  if (ct % prog == 0) Rcpp::Rcout << " % " << (float) ct / (float) nvox << std::endl;
	  ct++;
	  ...
	  switch( whichstat ) 
	    {
		 ...
		 stat=npdf(voxels,true,www);
		  if (ct == 1)	Rcpp::Rcout << "the max prob appearance \n";
			break;	
	       case 2:
		 stat=npdf(voxels,false,www);
		 if (ct == 1)	Rcpp::Rcout << "the probabilistically weighted appearance " << www << " \n";
		  
			break;

	       case 3:
		  stat=trimmean(voxels);	
		  if (ct == 1)	Rcpp::Rcout << "the trimmed mean appearance \n";
			break;

	       case 4:
		  stat=myantsmax(voxels);	
		  if (ct == 1)	Rcpp::Rcout << "the maximum appearance \n";
			break;
	       case 5:
		 stat=myantssimilaritymaxlabel(voxels,similarities,true);	
		  if (ct == 1)	Rcpp::Rcout << "the maximum similarity-based label \n";
			break;
	       case 6:
		 stat=myantssimilaritymaxlabel(voxels,similarities,false);	
		  if (ct == 1)	Rcpp::Rcout << "which image provides the maximum similarity-based label \n";
			break;
	       case 7:
		 stat=voxels[maxval]; 	
		  if (ct == 1)	Rcpp::Rcout << "which image provides the maximum similarity-based label \n";
	       ...
	       default:
		  stat=median(voxels);
		  if (ct == 1)	Rcpp::Rcout << "the median appearance \n";
			break;
	    }
	  float sval=stat;
...
RcppExport SEXP ImageSetStatistics( SEXP r_args )
try
{
  ...
  { 
    ...
    Rcpp::Rcout << argv[0] << " ImageDimension controlslist.txt outimage.nii whichstat {roi.nii} {imagelist2forsimilarityweightedstats.txt}" << std::endl; 
    Rcpp::Rcout << " whichstat = 0:  median,  1:  max prob appearance  , 2: weighted mean appearance ,  3: trimmed mean , 4 : max value , option 5 : similarity-weighted (must pass imagelist2 as well) else median , option 6 : same as similarity-weighted option 5 but the label corresponds to the image that provides the best local match ... useful if you want to MRF smooth these indices  , option 7 : similar to 5 but expects the max-value to be stored in the ROI image and uses it to get the intensity ... "  << std::endl;
    Rcpp::Rcout << " example:   ImageSetStatistics  3   imagelist.txt  maxvalueimage.nii.gz 4 " << std::endl;
    Rcpp::Rcout << " similarity weighted --- pass in a list of similarity images here which will be used to select the best label --- thus, number of similarity images must match the number of label images . " << std::endl;
