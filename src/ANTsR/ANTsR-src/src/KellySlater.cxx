...
//    double length=0.0;
 ...
 for(  iter.GoToBegin(); !iter.IsAtEnd(); ++iter )
    ...
    //    Rcpp::Rcout <<" difmag " << difmag << ": its " << ct <<  " len " << m_MFR->MeasureDeformation(inverseField ) << std::endl;
...
int LaplacianThicknessExpDiff2(int argc, char *argv[])
{
  ...
  Rcpp::Rcout <<" smooth " << smoothingsigma << " thp " << thickprior << " gs " << gradstep << std::endl;
  ...
  fmat.SetIdentity();
  Rcpp::Rcout <<" Setting Identity Direction  " << fmat << std::endl;
  ...
  Rcpp::Rcout << " Scaling done " << std::endl;
  ...
  while ( its < alltheits &&  badct < 4)
    {
      its++;
      if (totalerr > lasterr) { badct++; Rcpp::Rcout << " badct " << badct << std::endl; }
      else badct=0;
      lasterr=totalerr;
      ...
      while( ttiter < numtimepoints )  // N time integration points
	{
	  ...
	  if (debug) Rcpp::Rcout <<" exp " << std::endl;
	  ...
	  //	  Rcpp::Rcout  << " corrf len " << m_MFR->MeasureDeformation( corrfield ) << std::endl;
	  if (debug) Rcpp::Rcout <<" gmdef " << std::endl;
...
//speed function here IMPORTANT!!
	  ...
	  //	  Rcpp::Rcout << " maxlapgrad2mag " << maxlapgrad2mag << std::endl;
	  Iterator.GoToBegin();
	  while(  !Iterator.IsAtEnd()  )
	    {
	...
	while(  !Iterator.IsAtEnd()  )
	  {
	    ...
	    if ( thkval > 10 )
	      {
		Rcpp::Rcout << "thkval " << thkval << " hitval " << hitval << " total " << totalimage->GetPixel(velind) << std::endl;
	      }
      ...
      if (debug)  Rcpp::Rcout << " now smooth " << std::endl;
    ...
    Rcpp::Rcout << " error " << totalerr << " at it " << its  << " th-err " << thicknesserror/(RealType)thickerrct << " max thick " << maxth << std::endl;
//    std::string sulcthickname =outname + "sulcthick.nii";
    //    if (ImageDimension==2) WriteJpg<ImageType>(finalthickimage,"thick.jpg");
    //    std::string velofieldname = outname + "velofield";
    //WriteDisplacementField<DisplacementFieldType>(velofield,velofieldname.c_str());
    if (debug)Rcpp::Rcout << "outside it " << its << std::endl;
    //std::cin.get();
...
try
{
  ...
  if ( argc < 6)
    {
    Rcpp::Rcout << "Usage:   " << argv[0] << " ImageDimension Segmentation.nii.gz WMProb.nii.gz GMProb.nii.gz   Out.nii {GradStep-1-2D,2-3D}   {#Its-~50}  {ThickPriorValue-6} {Bool-use-curvature-prior} {smoothing} {BoolUseEuclidean?}" << std::endl;
    Rcpp::Rcout << " this is a kind of binary image registration thing with diffeomorphisms " << std::endl;
    Rcpp::Rcout << " Segmentation.nii.gz -- should contain the value 3 where WM exists and the value 2 where GM exists " << std::endl;
