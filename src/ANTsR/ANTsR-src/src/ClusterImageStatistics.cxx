...
//  const unsigned int ImageDimension = AvantsImageDimension;
  ...
  //  Rcpp::Rcout << " Cth " << clusterthresh << " Vth " << valuethresh << std::endl;
  ...
  for (unsigned int filecount=7;  filecount < argc; filecount++)
    {    
      //    Rcpp::Rcout <<" doing " << std::string(argv[filecount]) << std::endl;
    ...
    for(  tIter.GoToBegin(); !tIter.IsAtEnd(); ++tIter )
      if (roiimage->GetPixel(tIter.GetIndex()) < 0.5) tIter.Set(0);
...
//typename 
    ...
    //    Rcpp::Rcout << " #object " << maximum << std::endl;
//    float maxtstat=0;
	...
	Values->SetPixel( vfIter.GetIndex(), sumofvalues[(unsigned long)vfIter.Get()]/(float)histogram[(unsigned int)vfIter.Get()]  );
  ...
  //  Rcpp::Rcout << " max size " << maximgval << std::endl;
...
//  WriteImage<ImageType>(Labels,(outname+"labels.nii.gz").c_str());
  ...
  // now begin output 
  //  Rcpp::Rcout << " Writing Text File " << outname << std::endl;
  ...
  if (outf.good())
      ...
      for (int i=0; i < maximum+1; i++)
	{
	  if ( histogram[i] >= minSize)
	    {
	      //	      outf << " Cluster " << i << " size  " << histogram[i] <<  " average " << sumofvalues[i]/(float)histogram[i] << " max " << maxvalue[i] << " label " <<  maxlabel[i] <<  std::endl;
	      Rcpp::Rcout << " Cluster " << i << " size  " << histogram[i] <<  " average " << sumofvalues[i]/(float)histogram[i] << " max " << maxvalue[i] << " label " <<  maxlabel[i] <<  std::endl;
	    }
	}
      for (unsigned int i=0; i <= range; i++)
	{
	  //	  if ( countinlabel[i] > 0)
	    {
	      if (countinlabel[i] == 0) countinlabel[i]=1;
	      //	      outf << " Label " << i+min <<   " average " << suminlabel[i]/(float)countinlabel[i] <<  std::endl;
	      Rcpp::Rcout << " Label " << i+min <<   " average " << suminlabel[i]/(float)countinlabel[i] <<  std::endl;
	      if ( i >= 0 && i < range ) {  outf <<  suminlabel[i]/(float)countinlabel[i] << ","; }
	      else {  outf <<  suminlabel[i]/(float)countinlabel[i] << std::endl; }
	    }
	}
    }
  else Rcpp::Rcout <<" File No Good! " << outname << std::endl;
  outf.close();
  ...
  if (outf2.good())
  {
    for (unsigned int i=0; i <= range; i++) {
      if (countinlabel[i] == 0) countinlabel[i]=1;
        if ( i >= 0 && i < range ) {  outf2 <<  (float)countinlabel[i] << ","; }
          else {  outf2 <<  (float)countinlabel[i] << std::endl; }
    }
  }
  else Rcpp::Rcout <<" File No Good! " << outname << std::endl;
  outf2.close();
...
RcppExport SEXP ClusterImageStatistics( SEXP r_args )
try
{
