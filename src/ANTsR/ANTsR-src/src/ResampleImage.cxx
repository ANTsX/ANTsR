...
int ResampleImage( int argc, char *argv[] )
{
  ...
  if( argc <= 5 || atoi( argv[5] ) == 0 )
    ...
    if ( sp.size() == 1 )
      ...
      {
      Rcpp::Rcout << "Invalid spacing." << std::endl;
      }
    ...
    if ( sp.size() == 1 )
      ...
      {
      Rcpp::Rcout << "Invalid size." << std::endl;
      }
...
RcppExport SEXP ResampleImage( SEXP r_args )
try
{
  ...
  if ( argc < 5 )
    ...
    Rcpp::Rcout << "  Interpolation type: " << std::endl;
    Rcpp::Rcout << "    0. linear (default)" << std::endl;
    Rcpp::Rcout << "    1. nn " << std::endl;
    Rcpp::Rcout << "    2. gaussian [sigma=imageSpacing] [alpha=1.0]" << std::endl;
    Rcpp::Rcout << "    3. windowedSinc [type = 'c'osine, 'w'elch, 'b'lackman, 'l'anczos, 'h'amming]" << std::endl;
    Rcpp::Rcout << "    4. B-Spline [order=3]" << std::endl;
