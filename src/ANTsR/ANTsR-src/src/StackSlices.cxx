...
try
{
  ...
  if (argc < 5)
  {
    Rcpp::Rcout << "Usage: " << argv[0] << " outputvolume x y z inputvolumes" << std::endl;
  ...
  for (unsigned int i=0; i<3; i++)
    ...
    if (dimVars[i] > -1)
      ...
      if ( (dim > -1) || (slice > -1) )
	{
	Rcpp::Rcout << "Can only choose slice from 1 dimension" << std::endl;
	return Rcpp::wrap( EXIT_FAILURE ) ;
	}
      dim = i;
      slice = dimVars[i];
  ...
  mean/=ct;    Rcpp::Rcout << " Mean " << mean << std::endl;
  mean=1;
  it1.GoToBegin();
  while (!it1.IsAtEnd())
    {
  ...
  for (unsigned int i=1; i<nSlices; i++)
    ...
    Rcpp::Rcout <<" Slice "<< i << " :: " << std::string(argv[5+i]) << std::endl;
    //    Rcpp::Rcout << reader->GetOutput()->GetLargestPossibleRegion().GetSize() << std::endl;
