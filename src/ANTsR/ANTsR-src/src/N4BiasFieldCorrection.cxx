...
public:
  ...
  void Execute(const itk::Object * object, const itk::EventObject & event)
    ...
    if( filter->GetElapsedIterations() == 1 )
      {
      Rcpp::Rcout << "Current level = " << filter->GetCurrentLevel() + 1
        << std::endl;
      }
...
int N4( itk::ants::CommandLineParser *parser )
{
  ...
  if( inputImageOption )
    ...
    {
    Rcpp::Rcout << "Input image not specified." << std::endl;
    return EXIT_FAILURE;
    }
  ...
  if( !maskImage )
    {
    Rcpp::Rcout << "Mask not read.  Creating Otsu mask." << std::endl;
...
try
{
  ...
  Rcpp::Rcout << std::endl << "Running N4 for "
