...
//         thislength += totalmag;
       ...
       if ( ct >  1000 ) { Rcpp::Rcout << " stopping b/c exceed 1000 points " << voxmag <<  std::endl;  timedone=true;}
...
int IntegrateVectorField(int argc, char *argv[])
{
  ...
  while(  !Iterator.IsAtEnd()  )
      ...
      if ( ROIimage->GetPixel(velind) == 2 )
	{
	  ...
	  if ( (totalength)> 0 ) Rcpp::Rcout << " len1 " << len1 << " len2 " << len2 << " ind " << velind << std::endl;
	}
      ++Iterator;
    }
...
RcppExport SEXP ANTSIntegrateVectorField( SEXP r_args )
try
{
  ...
  if ( argc < 4)
    {
      Rcpp::Rcout << "Usage:   " << argv[0] << "  VecImageIN.nii.gz ROIMaskIN.nii.gz FibersOUT.vtk  LengthImageOUT.nii.gz   " << std::endl;
      Rcpp::Rcout << " The vector field should have vectors as voxels , the ROI is an integer image, fibers out will be vtk text files .... " << std::endl;
      Rcpp::Rcout << "  ROI-Mask controls where the integration is performed and the start point region ... " << std::endl;
      Rcpp::Rcout << " e.g. the brain will have value 1 , the ROI has value 2 , then all starting seed points " << std::endl;
      Rcpp::Rcout << " for the integration will start in the region labeled 2 and be constrained to the region labeled 1. "  << std::endl;
