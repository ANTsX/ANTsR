...
iMath <- function( img, operation , ... ) {
  ...
  if ( operation == "GetOperations" | operation == "GetOperationsFull" )
  {
    ops<-c(
      ...
      "  Project Image1.ext axis-a which-projection   : Project an image along axis a, which-projection=0(sum, 1=max, 2=min)","  G Image1.ext s    : Smooth with Gaussian of sigma = s",
    ...
    ops2<-c(" TimeSeriesAssemble : Outputs a 4D time-series image from a list of 3D volumes.",
            ...
            "bspline] [sincRadius=4 / bsplineOrder=3]",
            ...
            "  CropBinaryImage        : returns cropped image",
            ...
            "  SetOrGetPixel    :","LocalityVsGlobalityWeight-In-ZeroToOneRange OptionalPriorImages",
