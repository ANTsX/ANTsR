...
getMultivariateTemplateCoordinates <- function(
  ...
  for (x in 2:length(imageSetToBeLabeledIn)) {
    ...
    clust <- labelClusters(threshimg,
      minClusterSize = clustparam,
    ...
    subnet <- data.frame(
      ...
      temp2$templatepoints, pval = c(
        round(pvals[x - 1] * 1.e6) / 1.e6,
  ...
  return(list(
    networks = myout, myLabelsInTemplateSpace = temp2$myLabelsInTemplateSpace,
