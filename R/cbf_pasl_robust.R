cbf_pasl_robust <- function(asl_img, m0img, labelfirst = 0, robust = TRUE, lambda = 0.9, alpha = 0.95, TI1 = 700, 
  TI2 = 1700, T1b = 1664) {
  
  moco_results <- motion_correction(asl_img)
  moco_results_m0 <- motion_correction(m0img, fixed = moco_results$moco_avg_img)
  moco_mask_img <- get_mask(moco_results$moco_avg_img, lowThresh = 500, highThresh = 1e+09, cleanup = TRUE)
  ...
  deltaM <- perfusionregression(moco_mask_img, mat, predictors$xideal, predictors$nuis, moco_results_m0$moco_avg_img, 
    robust)
