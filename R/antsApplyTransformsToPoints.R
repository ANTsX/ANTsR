...
#' @param transformlist list of transforms generated by antsRegistration where
...
#' other interpolator available in antsApplyTransformsToPoints.
...
#' @param ... other parameters to pass to antsApplyTransformsToPoints.
...
#' mywarpedimage <- antsApplyTransformsToPoints( fixed=fixed,moving=moving,
#'   transformlist=mytx$fwdtransforms )
#' mywarpedimage <- antsApplyTransformsToPoints( fixed=moving,moving=fixed,
...
antsApplyTransformsToPoints <- function(
  ...
  if (missing(fixed) | missing(moving) | missing(transformlist)) {
    cat(" warpedimg<-antsApplyTransformsToPoints( fixed=img1 , moving=img2 , transformlist=list(\"my0GenericAffine.mat\",\"my1Warp.nii.gz\") ) ")
    cat("\n\n")
    cat("For full mode: use standard ants call as in antsApplyTransformsToPoints full mode ... \n\n")
    cat(" antsApplyTransformsToPoints(\"-d\",\"2\",\"-i\",\"r64slice.nii.gz\",\"-o\",\"temp.nii.gz\",\"-r\",\"r16slice.nii.gz\",\"-t\",\"./Z0GenericAffine.mat\") \n")
    cat("for full help: \n")
    cat("use .Call( \"antsApplyTransformsToPoints\", .int_antsProcessArguments( c(list(\"--help\")) ), PACKAGE=\"ANTsR\" );\n\n")
    return(0)
  }
  ...
  if (!is.character(fixed)) {
    if (fixed@class[[1]] == "antsImage" & moving@class[[1]] == "antsImage") {
      ...
      if (ttexists) {
        ...
        .Call("antsApplyTransformsToPoints", c(myargs, "-z", 1, "--float", 1, "-e",
          imagetype), PACKAGE = "ANTsR")
        return(antsImageClone(warpedmovout, inpixeltype))
      }
      if (!ttexists)
        cat("Problem in arg list \n see usage by calling antsApplyTransformsToPoints() w/o arguments \n")
    }
    return(0)
  }
  # if ( Sys.info()['sysname'] == 'XXX' ) { mycmd<-.antsrParseListToString( c(args)
  # ) system( paste('antsApplyTransformsToPoints ', mycmd$mystr ) ) return( antsImageRead(
