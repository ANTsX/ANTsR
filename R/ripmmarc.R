...
scaleSpaceFeatureDetection <- function(
    image, numberOfBlobsToExtract,
    minScale,
    maxScale,
    stepsPerOctave = 10,
    negate = FALSE) {
...
#' pr <- 2
#' nv <- 15
#' ripped <- ripmmarc(img, thresholdImage(mskTestTrain, 2, 2),
#'   patchRadius = pr,
...
#'
#' rippedTest <- ripmmarc(img, thresholdImage(mskTestTrain, 1, 1),
#'   patchRadius = pr,
...
ripmmarc <- function(
  ...
  if (is.null(evecBasis)) {
    evecBasis <- matrix(nrow = 0, ncol = 0)
...
#' }
#' set.seed(1234)
#' rp <- ripmmarcPop(pop, popmasks,
#'   patchRadius = 3,
#'   meanCenter = TRUE, patchSamples = 1000
#' )
#' set.seed(1234)
#' rp2 <- ripmmarcPop(pop, popmasks,
#'   patchRadius = 3,
#'   meanCenter = TRUE, patchSamples = 1000
#' )
#' testthat::expect_equal(rp, rp2)
#' rp3 <- ripmmarcPop(pop, popmasks,
#'   patchRadius = 3,
...
#' \dontrun{
#' nv <- 15
#' rippedTest <- ripmmarc(pop[[3]], popmasks[[3]],
#'   patchRadius = 3,
...
ripmmarcPop <- function(
  ...
  if (maskIsList) {
    ...
    randMask <- randomMask(mask, patchSamples, perLabel = TRUE, seed = seed)
  }
  ripped <- ripmmarc(ilist[[1]], randMask,
  ...
  ripmat <- getNeighborhoodInMask(ilist[[1]], randMask,
    radius = myrad,
    boundary.condition = "image"
  )
  for (i in 2:length(ilist)) {
    if (maskIsList) {
      randMask <- randomMask(mask[[i]], patchSamples,
        perLabel = TRUE,
        seed = seed
      )
    }
    locmat <- getNeighborhoodInMask(ilist[[i]], randMask,
      radius = myrad,
      boundary.condition = "image"
    )
