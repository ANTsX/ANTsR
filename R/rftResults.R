...
rftResults <- function(x, resels, fwhm, df, fieldType,
  ...
  if (class(threshType) == "character") {
    ...
    stop(paste0("threshType must be a numeric value or a character ", 
  ...
  dimnames(ClusterStats)[[2]] <- c("Pfwe", "Pfdr", "P", 
                                   "Voxels", "xc", "yc", "zc")
  ...
  K <- sapply(ClusterStats[, 4], function(tmp)(
    if (is.null(RPVImg)) {
      ...
      RPVImg = ANTsR::check_ants(RPVImg)
  ...
  PeakStats[, 4] <- sapply(labs, function(tmp)(
    max(x[clust == tmp]))) # max value for each cluster
  PeakStats[, 1] <- sapply(PeakStats[, 4], 
                           function(tmp)(
