#' Simple plotNetwork function.
#'
#' plotNetwork plots nodes in 3D space
#'
#' @param network input matrix
#' @param mask mask image
#' @param centroids matrix of point positions
#' @param N number of points
#' @param scaling scales the node size
...
.plotNetwork <- function(network, mask, centroids, N = 100, scaling = c(0, 0)) {
  ...
  surf <- as.array(mask)
  brain <- contour3d(surf, level = c(0.5), alpha = 0.3,
    draw = FALSE, smooth = 1,
