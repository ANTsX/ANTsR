renderSurfaceFunction <- function(surfimg, funcimg, surfval = 0.5, basefval, offsetfval, smoothsval = 0, smoothfval = 0, 
  blobrender = TRUE, alphasurf = 1, alphafunc = 1, outdir = "./", outfn = NA, mycol, physical = TRUE) {
  if (missing(surfimg)) {
  ...
  if (missing(funcimg)) {
    ...
    for (i in 1:length(surfimg)) {
      ...
      brain <- contour3d(surf, level = c(surfval[i]), alpha = alphasurf[i], draw = FALSE, smooth = 1, material = "metal", 
        depth = 0.6, color = "white")
      # each point has an ID, 3 points make a triangle , the points are laid out as c( x1 , y1, z1, x2, y2, z2 , ...
      # , xn, yn, zn ) indices are just numbers vertices<-c( brain <- subdivision3d(brain)
      if (physical == TRUE) {
  ...
  # s<-scene3d() s$par3d$windowRect <- c(0, 0, 500, 500) # make the window large 1.5*s$par3d$windowRect
...
makefacet <- function(data) {
  # Code for 3D function->stl files for molding and casting stl creation functions similar to week 4 files Laura
  # Perovich Oct 2012 Load package misc3d that includes surfaceTriangles function Define character constants
  # used in the stl files
  ...
  for (i in 1:nrow(data[[1]])) {
    v1 <- paste("  vertex", as.character(data[[1]][i, 1]), as.character(data[[1]][i, 2]), as.character(data[[1]][i, 
      3]), sep = " ")
    v2 <- paste("  vertex", as.character(data[[2]][i, 1]), as.character(data[[2]][i, 2]), as.character(data[[2]][i, 
      3]), sep = " ")
    v3 <- paste("  vertex", as.character(data[[3]][i, 1]), as.character(data[[3]][i, 2]), as.character(data[[3]][i, 
      3]), sep = " ")
...
makestl <- function(facetvector, outfile) {
  # Code for 3D function->stl files for molding and casting stl creation functions similar to week 4 files Laura
  # Perovich Oct 2012 Load package misc3d that includes surfaceTriangles function
  library(misc3d)
...
############################ to use this do ############################ library(ANTsR) source('R/renderSurfaceFunction.R')
############################ fn<-'/Users/stnava/Downloads/resimplerenderingexample/wmss.nii.gz' img<-antsImageRead(fn,3)
############################ brain<-renderSurfaceFunction( img ) fv<-makefacet(brain[[1]]) makestl(fv,'/tmp/temp.stl')
