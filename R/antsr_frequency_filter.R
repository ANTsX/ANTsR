antsr_frequency_filter <- function(values, tr, freqLo, freqHi, nuis = NA) {
  if ((class(values) != "matrix" && class(values) != "data.frame") || length(dim(values)) != 
    2) {
  ...
  # first calculate the filter width for the butterworth based on TR and the
  # desired frequency
  voxLo <- round((1/freqLo))  # remove anything below this (high-pass)
  voxHi <- round((1/freqHi))  # keep anything above this
  
  myTimeSeries <- ts(values, frequency = 1/tr)
  if (useNuis) {
  ...
  # print(paste('writing out reference filtering result',fn)) pdf(fn)
  vv <- 10
  par(mfrow = c(2, 2))
  # plot(myTimeSeries[,vv],type='l')
  spec.pgram(myTimeSeries[, vv], taper = 0, fast = FALSE, detrend = F, demean = F, 
    log = "n")
