sparseDecom2def <- function(inmatrix, inmask = c(NA, NA), sparseness = c(0.01, 0.01), 
  nvecs = 5, its = 5, cthresh = c(0, 0), statdir = NA, perms = 0, uselong = 0, 
  z = 0, smooth = 0, robust = 0, mycoption = 1, initializationList = list(), initializationList2 = list(), 
  ell1 = 0.05) {
  basecca <- sparseDecom2(inmatrix = inmatrix, inmask = inmask, sparseness = sparseness, 
    nvecs = 2, its = its, cthresh = cthresh, statdir = statdir, perms = perms, 
  ...
  for (j in 2:nvecs) {
    ...
    basecca <- sparseDecom2(inmatrix = list(inmat1r, inmat2r), inmask = inmask, 
      sparseness = sparseness, nvecs = 2, its = its, cthresh = cthresh, statdir = statdir, 
      perms = perms, uselong = uselong, z = z, smooth = smooth, robust = robust, 
      mycoption = mycoption, initializationList = initializationList, initializationList2 = initializationList2, 
      ell1 = ell1)
