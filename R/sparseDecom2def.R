...
{
  basecca<-sparseDecom2( inmatrix=inmatrix, inmask=inmask, sparseness=sparseness, nvecs=1, its=its, cthresh=cthresh, statdir=statdir, perms=perms, uselong=uselong, z=z, smooth=smooth, robust=robust, mycoption=mycoption,  initializationList=initializationList, initializationList2=initializationList2, ell1=ell1 )
  ...
  for ( j in 2:nvecs ) {
    ...
    basecca<-sparseDecom2( inmatrix=list(inmat1r,inmat2r), inmask=inmask, sparseness=sparseness, nvecs=1, its=its, cthresh=cthresh, statdir=statdir, perms=perms, uselong=uselong, z=z, smooth=smooth, robust=robust, mycoption=mycoption,  initializationList=initializationList, initializationList2=initializationList2, ell1=ell1 )
    deflatemat1[,j]<-basecca$projections[,1]
    deflatemat2[,j]<-basecca$projections2[,1]
    if ( typeof(basecca$eig1[[1]]) != "double" )  {
      print("...")
    }
    eig1<-cbind(eig1,basecca$eig1)
    eig2<-cbind(eig1,basecca$eig2)
  }
