...
#' @param
#' b-or-bspline=list(<numberOfLevels=6>,<initialMeshResolution=1x1x...>,<splineOrder=3>)
#' If the adaptive smoothing weights are > 0, the intensity images are smoothed
#' in calculating the likelihood values. This is to account for subtle
#' intensity differences across the same tissue regions.
#' @param i-or-initialization=
#'
...
#' with multivariate initialization. However, since a Euclidean distance on the
#' inter cluster distances is used, one might have to appropriately scale the
#' additional input images. Random initialization is meant purely for
...
#' model the partial voluming effects between csf/gm and gm/wm. The user would
#' specify it using -i kmeans[3] and -t 1x2 -t 2x3. So, for this example, there
...
#' asynchronous update step modulated by a specified annealing temperature. If
#' one sets the AnnealingTemperature > 1 in the posterior formulation a
#' traditional code set for a proper ICM update will be created. Otherwise, a
...
#' algorithm.
#' @param
#' c-or-convergence=list(<numberOfIterations=5>,<convergenceThreshold=0.001>)
#' Convergence is determined by calculating the mean maximum posterior
#' probability over the region of interest at each iteration. When this value
...
#' standard deviation for the Gaussian of each class is calculated at each
#' iteration. Other groups use non-parametric approaches exemplified by option
...
#' this number causes more smoothing whereas decreasing the number lessens the
#' smoothing. The radius parameter specifies the mrf neighborhood. Different
...
#' theoretical convergence properties.
#' @param
#' g-or-icm=list(<useAsynchronousUpdate=1>,<maximumNumberOfICMIterations=1>,<icmCodeImage=''>)
#' Asynchronous updating requires the construction of an ICM code image which
#' is a label image (with labels in the range [1,..,MaximumICMCode])
#' constructed such that no MRF neighborhood has duplicate ICM code labels.
#' Thus, to update the voxel class labels we iterate through the code labels
#' and, for each code label, we iterate through the image and update the voxel
#' class label that has the corresponding ICM code label. One can print out the
#' ICM code image by specifying an ITK-compatible image filename.
#' @param o-or-output=list(<classifiedImage>,<posteriorProbabilityImages>) The
#' output consists of a labeled image where each voxel in the masked region is
...
#' of the labels with the fast marching filter respects the distance along the
...
#' maximum value of 1.0 in the interior of the labeled region. The former
#' parameter dictates the exponential decay of probability propagation outside
#' the labeled region from the boundary probability, i.e.
#' boundaryProbability*exp( -lambda * distance ).
