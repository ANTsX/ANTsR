ants_compcorr_group <- function( moco_img = "" , bm_img = "" , cortmask_img = "" , mocoparams_csv = "" , dosvd = 0 )
{
	...
	sccan( "--timeseriesimage-to-matrix" , paste( "[" , paste( moco_img , cortmask_img , 3.0 , 0.0 , sep = "," ) , "]" , sep = "" ) , "-o" , csv ) ;

	compcorr_csv = paste( filename , "_compcorr_compcorr" , ".csv" , sep = "" ) ;
	filt_csv = paste( filename , "_filt" , ".csv" , sep = "" ) ;
	antsr_frequency_filter( csv , filt_csv , 3.0 , 0.1 , 0.25 , compcorr_csv ) ;

	if( dosvd == 1 )
	{
	 	RSF_Networks_img = paste( filename , "_RSF_Networks" , extension , sep = "" ) ;
		sccan( "--sparse-svd" , paste( "[" , paste( filt_csv , cortmask_img , -0.05 , sep = "," ) , "]" , sep = "" ) , "-n" , 40 , "-i" , 40 , "--PClusterThresh" , 100 , "-o" , RSF_Networks_img ) ;
