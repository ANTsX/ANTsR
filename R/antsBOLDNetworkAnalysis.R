antsBOLDNetworkAnalysis <- function( bold = NA, mask = NA, labels = NA, motion = NA ,
                               gdens=0.25, threshLo=1, threshHi=90, freqLo=0.01, freqHi=0.1,
                               winsortrim = 0.02 , throwaway=10 )
{
...
# question - should this be a constant of 0.2 as recommended in Yan Craddock He Milham?
    ...
    if ( winsortrim > 0 ) omat<-winsor(omat,trim=winsortrim)
    omat<-omat[keepinds,]
##################################################
    ...
    if ( bkgd )  mynuis<-cbind(scale(dmatrixm)[,1],scale(dtranm)[,1],classiccompcor, bgdnuis, templateFD[keepinds], DVARS[keepinds] ) else mynuis<-cbind(scale(dmatrixm)[,1],scale(dtranm)[,1],classiccompcor, templateFD[keepinds], DVARS[keepinds] )
    ...
    flo<-freqLo
    fhi<-freqHi
    mynetwork<-filterfMRIforNetworkAnalysis( mat , tr=antsGetSpacing(bold)[4], mask=aalmask ,cbfnetwork = "BOLD", labels= aalm , graphdensity = as.numeric(gdens), freqLo = flo, freqHi = fhi , usesvd=FALSE ) # , nuisancein = locmotnuis )
##################################################
############## now finalize it all ###############
##################################################
