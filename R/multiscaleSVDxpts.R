...
#' This function is useful for understanding the contribution of different features in the context of the SiMLR analysis. Assumes this input is generated by M3Embed.
#'
...
#' @examples
#' \dontrun{
#' M3Embed_update_residuals(mats, 1, "whiten", blaster2, allnna, 5)
#' M3Embed_update_residuals(opt = "opt")
#' }
#' @export
M3Embed_update_residuals <- function(mats, x, covariate, blaster2, allnna, n.comp, opt = NULL) {
...
#' @param covariates any covariates to adjust training matrices. if covariates is set to 'mean' then the rowwise mean will be factored out of each matrix.  this can be a vector e.g. \code{c('center','scale','rank')}. pass the name opt to M3Embed_update_residuals to have the function print the options.
...
M3Embed <- function(
  ...
  for (mycov in covariates) {
    ...
    for (x in 1:length(mats)) {
      ...
      mats[[x]] <- M3Embed_update_residuals(mats, x, mycov, blaster2, allnna, n.comp = nsimlr)
