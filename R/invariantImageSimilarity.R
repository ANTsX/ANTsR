...
#' @param mask optional fixed image mask
...
#' mival <- invariantImageSimilarity(fi, temp,
#'   thetas = c(0, 10, 20),
...
#' mival <- invariantImageSimilarity(fi, rotatedImage,
#'   thetas = c(0, 10, 20, 30, 40, 50),
...
invariantImageSimilarity <- function(
  ...
  if (doReflection == 0) {
    r1 <- ANTsRCore::invariantImageSimilarity(
      in_image1, in_image2,
