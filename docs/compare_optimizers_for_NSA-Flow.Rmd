...
#      print(paste("Running optimizer:", o, "with w =", round(w, 2)))
      ...
      if (!is.null(res)) {
        ...
        fid <- norm(res$Y - mat1, "F") # / norm(res$target, "F")
...
```{r ws_trends, fig.width=12, fig.height=12, echo=FALSE}
...
```{r pareto_plot, fig.width=5.5, echo=FALSE}
if (!"Optimizer" %in% names(summary_df)) {
  summary_df$Optimizer <- factor(seq_len(nrow(summary_df)))
  warning("Column 'Optimizer' not found — assigning placeholder labels.")
} else {
...
* **Without runtime:** prioritizes algorithmic accuracy — orthogonality and fidelity.
