...
myits <- 200
myopt <- "bidirectional_lookahead"
params_grid <- expand.grid(
  opt=c( "bidirectional_lookahead","adam", "nadam",
    "lookahead", 'armijo_gradient'),
  opt=c( "adam", "bidirectional_lookahead", 'armijo_gradient'),
#  energyType = c('normalized_correlation', "acc", "regression"),
  energyType = c( "reg", 'nc', 'acc' ),
  mixAlg = c("pca", "ica", 'svd'),
  constraint = c("orthox0x10", "orthox0.25x10","orthox0.5x10","orthox0.75x10","orthox1x10", 'none'),
  domainLambda = c( 1.0, 0.0 ) 
)
...
# Permutation test
perm_test <- function(result, matlist_train, dlist, n_perm = 10, domlam, constraint, optimizer ) {
...
for (row in 1:nrow(params_grid)) {
  v = FALSE
  par <- params_grid[row, ]
  key <- paste(par$energyType, par$mixAlg, par$domainLambda, 
    par$opt, par$constraint, sep="_")
  ...
  result <- simlr(
    ...
    constraint = as.character(par$constraint),
    optimizationStyle = as.character(par$opt),
    verbose = v,
    domainMatrices = dlist,
    domainLambdas = rep(par$domainLambda, length(matlist_train)),
    randomSeed = 123
  )
  plot_energy_decomposition( result, "cortex"  )
  ...
  if (par$domainLambda > 0 & FALSE ) {
    perm_res <- perm_test(result, matlist_train, dlist, n_perm = 5, 
      domlam = par$domainLambda, 
      constraint = as.character(par$constraint),
      optimizer  = as.character(par$opt) )
...
plots <- lapply(1:nrow(top_configs), function(i) {
  key <- paste(top_configs$energyType[i], top_configs$mixAlg[i], top_configs$domainLambda[i], 
               top_configs$opt[i], top_configs$constraint[i], sep="_")
...
```{r domain-example, fig.width=8, fig.height=6,eval=FALSE}
