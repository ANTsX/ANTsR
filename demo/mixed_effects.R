...
for (x in c(1:length(fnl))) {
  ...
  newbdate <- paste(substr(demog$DOB[wh], 1, 7), "19", substr(demog$DOB[wh], 8, 
    12), sep = "")
  newt1date <- paste(substr(demog$Time1[wh], 1, 7), "20", substr(demog$Time1[wh], 
    8, 12), sep = "")
  newt2date <- paste(substr(demog$Time2[wh], 1, 7), "20", substr(demog$Time2[wh], 
    8, 12), sep = "")
  if (!is.na(demog$FTDYearOnset[wh])) 
    duration[x] <- as.real(substr(newt1date, 8, 12)) - as.real(demog$FTDYearOnset[wh]) + 
      0.5
  dx[x] <- demog$X[wh]
  ...
  if ((x%%2 == 0)) {
    if (!is.na(demog$FTDYearOnset[wh])) 
      duration[x] <- as.real(substr(newt2date, 8, 12)) - as.real(demog$FTDYearOnset[wh]) + 
        0.5
...
vox <- 1
while (vox <= mxtest) {
  mydatafrm <- data.frame(age = age[inds], tau = tauabeta[inds], dtime = dtime[inds], 
    dx = dx[inds], edu = edu[inds], jacobian = mydatadif[, vox], subjid = subjid[inds], 
    duration = duration[inds])
  fm1 <- lm(jacobian ~ dx + dtime + edu, data = mydatafrm)
  # try( fm1<-lmrob( jacobian ~ dx + age + dtime + edu , data = mydatafrm , control
  # = ctl ) )
  try(pvals[vox] <- summary(fm1)$coeff[2, 4])
  if (vox%%1000 == 0) {
    densityplot(mydatafrm$jacobian, groups = mydatafrm$dx)
    print(summary(fm1))
    print(vox/nmatcol * 100)
    print(min(p.adjust(pvals, method = "BH"), na.rm = T))
  }
  vox <- vox + 1
}
...
ctl <- lmeControl(maxIter = 10000, msMaxIter = 10000, tolerance = 0.001, msVerbose = FALSE, 
  returnObject = TRUE)
mydatafrm <- data.frame(age = age, duration = duration, dtime = dtime, dx = dx, edu = edu, 
  jacobian = mydata[, vox], subjid = subjid)
vox <- 1
while (vox <= mxtest) {
  jacobian = mydata[, vox]
  ...
  pvals[vox] <- myanv$P[[2]]  # 
  if (vox%%1000 == 0) {
    densityplot(jacobian, groups = dx)
    print(summary(fm2))
    print(vox/nmatcol * 100)
    print(min(p.adjust(pvals, method = "BH"), na.rm = T))
  }
