...
for (asl in c("103487_20060308_0007_ep2d_casl_UI_1500ms.nii.gz", "111338_20060830_0006_ep2d_casl_UI_1500ms.nii.gz", 
  "110284_20060425_0009_ep2d_casl_UI_1500ms.nii.gz")) {
  pre <- substr(asl, 1, 6)
  moco_results <- motion_correction(asl)
  moco_mask_img <- get_mask(moco_results$moco_avg_img, thresh_lo = 800, thresh_hi = 1e+09)
  ...
  cbf <- perfusionregression(moco_mask_img, mat, predictors$xideal, predictors$nuis, 
    m0)
  cbfr <- perfusionregression(moco_mask_img, mat, predictors$xideal, predictors$nuis, 
    m0, 0.96)
  antsImageWrite(cbf, paste(pre, "cbf.nii.gz", sep = ""))
  antsImageWrite(cbfr, paste(pre, "cbfr.nii.gz", sep = ""))
}
# look at the results --- this is currently what we can use in a study if we
# control for global mean perfusion --- it's a relative measure can also do
# robust cbf


# network analysis draw the roi in label 2 on the mask
...
tr <- 4
myTimeSeries <- ts(mat, frequency = 1/tr)
filteredTimeSeries <- residuals(cffilter(myTimeSeries, pl = voxHi, pu = voxLo, drift = T, 
  type = "t"))
spec.pgram(filteredTimeSeries[, vv], taper = 0, fast = FALSE, detrend = F, demean = F, 
  log = "n")
...
# finally, try to convert to 'true' cbf values - not verified for all asl types!
# this assumes CASL
dd <- (moco_mask_img > 0)
...
omega <- 1
tau <- 2
realcbf <- (lambda * cbf2)/(2 * alpha * M0 * T1b * (exp(-omega/T1b) - exp(-(tau + 
  omega)/T1b)))
max(realcbf)
cbf[dd] <- realcbf
